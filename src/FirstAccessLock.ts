...
export class FirstAccessLock {
  private extendTimer?: NodeJS.Timeout;
  private extendAttempts = 0;
  ...
  async acquire(): Promise<boolean> {
    try {
      const acquired = await this.redisCommon.acquireLock(
        this.lockKey,
        this.nodeId,
   ...
   */
  async isHoldingLock(): Promise<boolean> {
    try {
  ...
  private startExtendTimer(): void {
    if (!this.options.extendIntervalMs) return;
